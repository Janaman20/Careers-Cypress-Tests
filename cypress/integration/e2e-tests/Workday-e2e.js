...
describe('workday e2e', () => {
    ...
    it(`search for job`, () => {
        // cy.setCookie('PLAY_SESSION', '780cd864a42cfdb649270c63fdd6a246a65a5a52-nandos_pSessionId=67mg5m47e8gh63uaopfdeuebuj&instance=wd3prvps0002i')
        cy.viewport("macbook-16")
        cy.visit("https://nandos.wd3.myworkdayjobs.com/Nandos")
        cy.waitFor(6000)
        cy.get('.WMJH > .gwt-TextBox', { timeout: 10000 })
            .should('be.visible')
            .type('N14SN') // Yield 'footer' within '.article'
        cy.get('.WFH0 > .gwt-Label').click()
        cy.waitFor(6000)
        cy.get('#promptOption-gwt-uid-9').click({force: true})
        cy.wait(6000)
        cy.get('[title="Nando\'s Kentish Town - Kentish Town, London, NW5 2JU"]')
            .click()
        cy.waitFor(5000)
        cy.get('#promptOption-gwt-uid-231').click()
        cy.waitFor(6000)
        cy.title( { timeout: 10000 }).should('eq','Front of House Team Member')
        cy.waitFor(6000)
        ...
        cy.waitFor(6000)
        cy.title({ timeout: 10000 }).should('eq','Front of House Team Member : My Information')
        cy.get('#input-1')
            .click()
        cy.wait(3000)
        cy.contains('[data-automation-label="Agency"]', 'Agency').click()
        cy.wait(3000)
        cy.contains('[data-automation-label="Focus Cloud Ltd (Agency)"]', 'Focus Cloud Ltd (Agency)').click()
        ...
        cy.title().should('eq','Front of House Team Member : My Experience')
        ...
        cy.get('button').contains('Next').click()
        cy.get('div > div.css-12izqjd > h2')
            .should("contain",
            "Application Questions")
        cy.get('#input-662').click()
        cy.get('[data-value="62bdf6cda978010f2b08739d480101b2"]')
            .should('be.visible')
            .contains('Yes')
            .click()
